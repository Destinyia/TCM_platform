...
| 前端 | Vue 3 + Element Plus + ECharts + Vite | 管理页、详情页、矩阵、图表、局部交互 | 当前仓库已采用 Vue 3 实现，后续以现有前端为准 |
...
为避免后续每次重新定位数据结构、路由和接口，当前仓库约定：

1. 每次修改代码、接口或数据结构后，必须同步更新相关文档。
2. 文档更新和代码修改必须在同一个 commit 中提交。
3. 修改 API 时同步更新 `docs/api.md`。
4. 修改前端路由、页面职责或展示聚合规则时同步更新 `docs/开发计划.md` 和本文。
5. 修改 ORM、SQL、seed 数据或本地存储结构时同步更新 `docs/开发计划.md`、`docs/标准化本地存储结构设计.md` 或相关数据结构文档。
6. commit 时只纳入本次相关文件，不提交本地 dump、临时文件、原始数据和虚拟环境。
...
后续定位数据结构、路由和页面时，优先查看以下文件：

| 类型 | 位置 | 说明 |
| --- | --- | --- |
| ORM 数据结构 | `backend/app/models.py` | `dim_user`、`fact_visit`、`fact_modality_record`、`fact_file_asset`、`fact_quality_event`、`mart_user_day_panel`、`dataset_version` |
| Demo API | `backend/app/demo_api.py` | `/api/demo/*` 页面数据接口 |
| Ingest API | `backend/app/ingest_api.py` | `/api/ingest/*` 结构化解析和标准存储入口 |
| Pulse Analysis API | `backend/app/pulse_api.py` | `/api/pulse/*` 下游脉诊研究辅助接口 |
| Flask 注册 | `backend/app/__init__.py` | `/api/demo`、`/api/ingest`、`/api/pulse`、`/api/health` |
| 前端路由 | `frontend/src/router/index.js` | 页面 URL 与 Vue 组件映射 |
| 前端 API 封装 | `frontend/src/services/api.js` | 页面调用的 API 方法 |
| 打卡矩阵页面 | `frontend/src/views/CheckinMatrix.vue` | 用户 × 日期矩阵，前端聚合早中晚 |
| 患者详情页面 | `frontend/src/views/PatientDetail.vue` | `/patients/:id` 用户纵向随访 |
| 脉诊分析页面 | `frontend/src/views/PulseAnalysis.vue` | 下游分析模块 |
...
| `/pulse-analysis` | `PulseAnalysis.vue` | 脉诊分析 |
| `/import` | `Import.vue` | 导入入口/说明 |
| `/export` | `Export.vue` | 导出与数据集版本 |
...
| `GET /api/demo/checkin-matrix` | 打卡矩阵 |
| `GET /api/demo/users` | 用户列表 |
| `GET /api/demo/users/{user_id}/timeline` | 患者随访详情 |
| `GET /api/demo/visits` | visit 列表 |
| `GET /api/demo/visits/{visit_id}` | visit 详情 |
| `GET /api/demo/assets` | 资产列表 |
| `GET /api/demo/assets/{asset_id}/file` | 文件资产读取 |
| `GET /api/demo/quality-events` | 质量事件 |
| `GET /api/demo/dataset-versions` | 数据集版本 |
| `GET /api/demo/pulse/records` | 脉诊记录明细 |
| `GET /api/demo/pulse/user-trend` | 单用户脉诊趋势 |
| `GET /api/demo/pulse/slot-stability` | 同槽位稳定性 |
| `GET /api/demo/pulse/cross-user` | 多用户横向对比 |
| `GET /api/demo/pulse/feature-drift` | 不同时段变化 |
| `POST /api/ingest/standard-storage` | 标准存储入口 |
| `POST /api/ingest/parse-structured-data` | 结构化解析入口 |
| `GET /api/pulse/analysis/user-summary` | 患者级脉诊在线摘要 |
| `GET /api/pulse/analysis/period-consistency` | 单记录原始波形周期一致性、半/倍周期候选与双角色模板 |

脉诊记录接口中的 `included` 保留为旧平台筛选兼容字段。专项页面应使用 `source_review_status` 展示来源复核问题，使用脉波质量规则输出质量等级，并使用 `research_inclusion_status` 表达 `eligible`、`dedup_review_required` 或 `insufficient_pulse_data` 的研究处理策略。

### 打卡矩阵当前展示规则

后端 `GET /api/demo/checkin-matrix` 仍按 `用户 -> 日期 -> 早/中/晚` 返回时段级单元格。当前前端展示在 `CheckinMatrix.vue` 中按日期聚合早中晚：

- 展示维度为 `用户 × 日期`。
- 单元格只保留 `数字`、`有效/无效`、`来源`。
- `count` 为当天早中晚记录数之和。
- `valid_count` 为当天有效记录数之和。
- 当 `count == valid_count` 时显示 `有效`，否则显示 `无效`。
- 无打卡的空单元格留空，不显示 `0`、`无效` 或来源占位。
- 缩略图和展开视图使用相同布局，只调整单元格密度。
- 姓名区域可点击进入 `/patients/{user_id}`。
...
- 同一用户同一时段稳定性散点图或箱线图。
- 多用户特征雷达图或柱状对比图。
- 脉象记录明细表，分别展示来源复核状态、脉波质量等级和专项研究纳入状态；疑似重复来源保留供去重复核，不直接覆盖脉波质量。
- 患者详情页单记录原始波形周期一致性表，展示 PulseNumbers 对齐模板、半/倍周期候选及倍周期形态风险提示。
...
| `GET /api/pulse/analysis/user-summary` | 患者 summary 在线分析 |
| `GET /api/pulse/analysis/period-consistency` | 原始波形周期一致性与倍周期模式展示 |
...
当前前端采用 Vue 3、Element Plus、ECharts 和 Vite：

- Vue 3 负责页面组件、路由、状态和交互。
- Element Plus 负责布局、表格、标签、按钮、表单、抽屉等管理端 UI。
- ECharts 负责趋势图、对比图、脉诊分析图表和后续 benchmark 指标图。
- Vite 负责本地开发和构建。
- 前端不内置业务 mock 数据，页面数据必须通过 Flask API 动态读取。
...
| 用户纵向随访 | 用户列表、患者随访详情、visit 时间线 |
| 打卡矩阵 | 用户 × 日期矩阵，前端聚合早中晚，只展示次数、有效性、来源 |
...
已新增独立的“打卡矩阵”页面 `/checkin-matrix`，通过 `GET /api/demo/checkin-matrix` 动态读取数据库中的 visit 数据。后端返回用户、日期、早中晚时段级结构，前端当前按用户和日期聚合早中晚，仅展示次数、有效/无效和来源。该视图后续可直接对应 `mart_user_day_panel` 或独立的 `mart_checkin_matrix`，作为离线整理质量验收页面。
